...
model_CE = torch.load(r'./model/model_CE_res_20.pth').to(device)
model_E_res = torch.load(r'./model/model_CE_res_20.pth').to(device)
model_E_res_ema = torch.load(r'./model/model_CE_res_20.pth').to(device)
model_D_res = torch.load(r'./model/model_CD_res_20.pth').to(device)
...
def train(e):
    for iteration, (batch1, batch2) in enumerate(zip(training_predata_loader, training_data_loader), 1):
        imgA, imgB, imgC = batch1[0].to(device), batch1[1].to(device), batch2.to(device)
        imgB = RGB2Y(imgB)
        imgC = RGB2Y(imgC)
        ...
        loss4 = loss_res_F(F, VIS, emb_F, emb_vis_c, emb_ema_vis_F, emb_F_1)
